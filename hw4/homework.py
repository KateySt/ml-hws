...
"""
"""
У даному випадку було використано метрику accuracy для оцінки якості моделі. Вибір цієї метрики обґрунтований тим, що задача класифікації зображень цифр є добре збалансованою, а точність надає чітке уявлення про загальну ефективність моделі, враховуючи всі класи.

При тестуванні різних значень параметра penalty (l1, l2, elasticnet, None), модель LogisticRegression показала гарні результати з параметром None, де точність на тестових даних досягла 93%. Кожен фолд крос-валідації демонстрував подібні результати, що свідчить про стабільність моделі на різних підмножинах даних. Звіти по кожному фолду продемонстрували хорошу збалансованість між точністю та відгуком для кожної категорії, що підтверджує ефективність вибору цієї моделі для вирішення задачі.
"""
